export default {
  user: {
    email: "toluwani@epicmail.com",
    secondaryEmail: "Okanlawon@gmail.com",
    firstName: "Malcolm",
    lastName: "Jude",
    password: "Password2019#",
  },

  shortPasswordLength: {
    email: "eleniyan@epicmail.com",
    secondaryEmail: "Chuks@gmail.com",
    firstName: "runeleniyan",
    lastName: "now",
    password: "3@$%#",
  },

  noFirstName: {
    email: "jedidiah@epicmail.com",
    secondaryEmail: "Okanlawon@gmail.com",
    firstName: "to1234",
    lastName: "Jude",
    password: "Password2019#",
  },

  userInputLessThan3: {
    email: "toluwani@epicmail.com",
    secondaryEmail: "Okanlawon@gmail.com",
    firstName: "Malcolm",
    lastName: "ab",
    password: "Password2019##",
  },

  userLogin: {
    email: "toluwani@epicmail.com",
    password: "Password2019#",
  },

  faileduserLogin: {
    email: "toluwani@epicmail.com",
    password: "Pfakepasswor234d#",
  },

  noPasswordLogin: {
    email: "toluwani@epicmail.com",
  },

  invalidEmailLogin: {
    email: "toluwani@epicmailcom",
    password: "Pfakepass234word#",
  },

  incompleteUser: {
    email: "toluwani@epicmail.com",
    firstName: "Malcolm",
    password: "Password2019#",
  },

  wrongRegExpName: {
    email: "toluwani@epic.",
    firstName: "Habeeb",
    lastName: "Whyte",
    password: "Password2019#",
    secondaryEmail: "talk@talki.com",
  },

  oneMissingDetail: {
    firstName: "Habeeb",
    lastName: "Whyte",
    password: "Password2019#",
    secondaryEmail: "talk@talki.com",
  },

  incompleteLogin: {
    password: "Password2019#",
  },

  eichUser: {
    email: "EeichBalkmna@epicmail.com",
    secondaryEmail: "eich@gmail.com",
    firstName: "Malcolm",
    lastName: "Balkman",
    password: "PO0cinwel#",
  },

  biodunUser: {
    email: "Biodun@epicmail.com",
    secondaryEmail: "goodguy@gmail.com",
    firstName: "Abiodun",
    lastName: "forWhat",
    password: "Password2019#",
  },

  passwordReset: {
    email: "Biodun@epicmail.com",
  },

  invalidEmailReset: {
    email: "Biodunepicmail.com",
  },

  fakeEmail: {
    email: "brisket@gmail.com",
  },

  loginEich: {
    email: "EeichBalkmna@epicmail.com",
    password: "Password2019#",
  },

  eichsMessage: {
    subject: "Mediocrity at Felmish",
    message:
      "I really found the level of mediocrity at a so-called Flemish company very disappointing",
    receiverEmail: "alfred@epicmail.com",
  },

  eichsOtherMessage: {
    subject: "Mediocrity at Felmish",
    message:
      "I really found the level of mediocrity at a so-called Flemish company very disappointing",
    receiverEmail: "alfred@remial.com",
  },

  alfred: {
    email: "alfred@epicmail.com",
    secondaryEmail: "eich@gmail.com",
    firstName: "Malcolm",
    lastName: "Balkman",
    password: "Password2019#",
  },

  alfredLogin: {
    email: "alfred@epicmail.com",
    password: "Password2019#",
  },

  eichsDraft: {
    subject: "Burundi of the future",
    message: "Why does nobody ever talk about Burundi?",
  },

  eichsBadMessage: {
    subject: "Burundi of the future",
    receiverEmail: "kolawsi@gmail.com",
  },

  eichsInvalidMessage: {
    subject: "Burundi of the future",
    receiverEmail: "kolawsi@gmailcom",
    message: "Welcome to Burundi, the land of the future",
  },

  brenda: {
    email: "brendafasey@epicmail.com",
    secondaryEmail: "BrendaMaconkey@gmail.com",
    firstName: "BRENDA",
    lastName: "Mckonckey",
    password: "Password2019##",
  },

  brendaMessage: {
    subject: "Dev meetup in Brazil",
    message:
      "You know we could actually make this more than a dev-meetup in Brazil, it would be my first time there",
    receiverEmail: "alfred@epicmail.com",
  },

  brendaMessageToAlfred: {
    subject: "Meet me in Brazil Eich now ",
    message:
      "Lets live life together, You know we could actually make this more than a dev-meetup in Brazil, it would be my first time there",
    receiverEmail: "alfred@epicmail.com",
  },

  brendaSavesDraft: {
    subject: "This is a draft message",
    message:
      "I brenda, decided to test the functionality of the draft endpoint at the this moment and see how effective this draft endpoint is at saving messages",
  },

  FailedUpdateBrendasDraft: {
    subject: "This is a draft message, haha",
    message:
      "I brenda, decided to test the functionality of the draft endpoint at the this moment and see how effective this draft endpoint is at saving messages",
    receiveremail: "alfred@epicmail.com",
  },

  updateBrendasDraft: {
    subject: "This is a draft message, haha",
    message:
      "I brenda, decided to test the functionality of the draft endpoint at the this moment and see how effective this draft endpoint is at saving messages",
    receiveremail: "tolu@epicmail.com",
  },

  tolumide: {
    email: "tolu@epicmail.com",
    secondaryEmail: "BrendaMaconkey@gmail.com",
    firstName: "Michael",
    lastName: "Mckonckey",
    password: "Password2019#",
  },

  Basheer: {
    email: "olabash@epicmail.com",
    secondaryEmail: "ALhamdiddd@gmail.com",
    firstName: "Aminah",
    lastName: "Saelif",
    password: "Password2019#",
  },

  chwuks: {
    email: "chukwudi@epicmail.com",
    secondaryEmail: "Ume@gmail.com",
    firstName: "Rayban",
    lastName: "Kamir",
    password: "Password2019#",
  },

  incompleteGroupDetails: {
    name: "Developers in Michigan",
  },

  chwuksGroup: {
    name: "Michigan developers",
    role: "Admin",
  },

  roleLength: {
    name: "Michigan developers",
    role: `The purpose of this test is to ascertain 
    that the validation can easily reject any group 
    length than is more than over fifty. Hopefully this would be`,
  },

  basheerGroup: {
    name: "Arab Money",
    role: "Admin",
  },

  editBasheerGroupName: {
    name: "Saudi Prince",
  },

  tomiwa: {
    email: "tomiwas@epicmail.com",
    secondaryEmail: "tomi@gmail.com",
    firstName: "tomiwa",
    lastName: "tomiwas",
    password: "Password2019#",
  },

  bambam: {
    email: "bambam@epicmail.com",
    secondaryEmail: "teddy@gmail.com",
    firstName: "teddy",
    lastName: "bambam",
    password: "Password2019#",
  },

  bambamAddToGroup: {
    userEmailAddress: "bambam@epicmail.com",
    userRole: "Guest",
  },

  tomiwaAddToGroup: {
    userEmailAddress: "tomiwas@epicmail.com",
    userRole: "Guest",
  },

  invalidEmailToTheGroup: {
    userEmailAddress: "tomiwasepicmail.com",
    userRole: "Guest",
  },

  invalidGroupRole: {
    userEmailAddress: "bambam@epicmail.com",
    userRole: "@#$%^&*()_",
  },

  unRegisteredMember: {
    userEmailAddress: "oladele@gmail.com",
    userRole: "Guest",
  },

  validationMember: {
    userEmailAddress: "oladele@gmail.com",
  },

  alfredAddToGroup: {
    userEmailAddress: "alfred@epicmail.com",
    userRole: "Guest",
  },

  broadcastMessage: {
    subject: "Welcome to Addis ababa, ore me",
    message: "Let us meet at the Cafe restaurant by 6pm, trust you are fine",
  },

  anotherBroadcastMessage: {
    subject: "Welcome to Southesiaas",
    message: "The island og the bela",
  },

  broadcastMessageValidation: {
    subject: "Arkansas",
  },

  emptyBroadcastMessage: {},

  shortBroadcastMessage: {
    subject: "We",
    message: "Le",
  },
};
